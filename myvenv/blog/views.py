...
def portalhtml(request):
    portalhtmls = Portalhtml.objects.all()
...
def savehtml(request):
    if request.method == "POST":
        try:
            html = Portalhtml.objects.get(author = request.user)
